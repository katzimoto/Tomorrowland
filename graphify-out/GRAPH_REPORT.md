...
Cohesion: 0.06
Nodes (52): _chunk_by_token_estimate(), chunk_text(), _ensure_max_tokens(), _estimate_tokens(), _find_chunk_positions(), Token-based text chunking with sentence-boundary awareness., Split *text* into sentences using the pattern for *language*., Map each chunk to its page_number / section_heading from *location_segments*. (+44 more)
...
Cohesion: 0.24
Nodes (8): availableVersion, onSelect, pendingVersion, qc, renderWithClient(), { rerender }, TranslationVersionSelector(), TranslationVersionSelectorProps
...
Cohesion: 0.14
Nodes (32): _admin(), _chunk(), _mock_task(), _no_group_user(), _provider(), _query(), source_ids filter must be ANDed with ACL filter, not replace it., _search_rag_hit() (+24 more)

### Community 47 - "Module Group 47"
Cohesion: 0.21
Nodes (18): BackfillService, BackfillSummary, _build_arg_parser(), _log_summary(), main(), _make_engine(), _mock_doc_row(), _mock_group_rows() (+10 more)
...
Cohesion: 0.32
Nodes (20): _admin_token(), _create_source_with_doc(), _setup_users(), test_download_returns_nosniff_header(), test_me_activity_empty_for_new_user(), test_me_activity_orders_by_most_recent(), test_me_activity_returns_view_history(), test_preview_archive_lists_filenames() (+12 more)
...
Cohesion: 0.09
Nodes (21): ChatPage(), parseScopeFromSearch(), ASSISTANT_REPLY, deleteBtns, input, mockNavigate, mockSearchParams, NEW_SESSION (+13 more)
...
Cohesion: 0.25
Nodes (12): ApiError, ApiRequestInit, AuthRedirectHandler, defaultAuthRedirectHandler(), getToken(), redirectToExpiredLogin(), request(), resetAuthRedirectHandler() (+4 more)
...
Cohesion: 0.18
Nodes (14): Elasticsearch, Qdrant Vector Search, ACL Audit Document, H2: /expertise Admin Bypass Broken, H4: /expertise Subscription User-Discovery Leak, H3: chunk_text Leak via Orphaned Qdrant Vectors, Permission matrix, H5: /related Uses Narrower Groups than /search (+6 more)
...
Cohesion: 0.05
Nodes (43): _ocr_pdf(), PdfExtractor, PDF text extractor.  Extracts text via pypdf.  When pypdf returns empty text for, Extract text from PDF files using pypdf., Return concatenated text from all pages., Render each PDF page as an image and OCR it.      Returns an empty string when `, Extract text from PDF files.      Uses pypdf for native text extraction.  Falls, Return concatenated text from all pages with page-number segments. (+35 more)
...
Cohesion: 0.40
Nodes (11): _admin_token(), _make_row(), Connector factory registry tests., _setup_admin(), test_admin_connector_types_endpoint_includes_smb(), test_admin_source_creation_accepts_smb(), test_admin_source_languages_endpoint_default_includes_major_languages(), test_admin_source_languages_endpoint_returns_configured_languages() (+3 more)
...
Cohesion: 0.19
Nodes (10): AppShell(), AppShellProps, ADMIN_ITEM, NAV_ITEMS, NavItem, NavKey, NavRail(), NavRailProps (+2 more)
...
Cohesion: 0.15
Nodes (12): Durable pipeline job runner that claims jobs from the queue and processes them., Process a ``process_document`` job end-to-end., Process an ``intelligence_document`` job (best-effort)., Process an ``intelligence_document`` job (best-effort)., Process an ``alert_document`` job (best-effort)., Process an ``alert_document`` job (best-effort)., Run ``run_once`` in a loop until interrupted.      Emits a heartbeat gauge and q, Run ``run_once`` in a loop until interrupted.      Emits a heartbeat gauge and q (+4 more)
...
Cohesion: 0.08
Nodes (24): Extractor, from_dict(), LocationSegment, Text extraction abstraction., Boundary for file-type-specific text extractors., Character-range location metadata for a contiguous span of extracted text., Return plain text extracted from the file at *path*.          Returns an empty s, Boundary for file-type-specific text extractors. (+16 more)
...
Cohesion: 0.38
Nodes (8): cleanup(), curl_json(), json_get(), log_step(), require_command(), usage(), wait_for_search_result(), wait_for_url()

### Community 178 - "Module Group 178"
Cohesion: 0.05
Nodes (34): Acceptance criteria, code:block1 (index_worker  → Meilisearch (BM25 only)), code:yaml (embed-worker:), code:python (# Pseudocode), code:md (## [Unreleased]), code:block2 (index_worker  → Meilisearch (BM25 + vectors — embedder calls), code:json ({), code:python (feature_meilisearch_hybrid: bool = False) (+26 more)
...
Cohesion: 0.06
Nodes (34): Acceptance Checklist, After This PR — Final Integration, Allowed Changes, CHANGELOG.md, code:bash (git fetch origin), code:markdown (## [Unreleased]), code:bash (git commit -m "feat(rabbit): air-gap compose and image manif), code:block12 (Co-Authored-By: DeepSeek <noreply@deepseek.com>) (+26 more)

### Community 192 - "Module Group 192"
Cohesion: 0.31
Nodes (7): Regression: MockEncoder must not appear in production source code., Regression: encoder classes must only be constructed inside build_encoder., Regression: API routers must wire encoders through build_encoder, never directly, _source_files(), test_api_routers_use_build_encoder(), test_no_direct_encoder_construction_outside_factory(), test_no_mock_encoder_in_source()
...
Cohesion: 0.06
Nodes (34): Acceptance Checklist, `AlertConsumer`, all pass, Allowed Changes, clean, code:bash (git fetch origin), code:bash (git commit -m "feat(rabbit): IntelligenceConsumer and AlertC), code:block11 (Co-Authored-By: DeepSeek <noreply@deepseek.com>) (+26 more)
...
Cohesion: 0.06
Nodes (33): 4 passed, Acceptance Checklist, all pass, Allowed Changes, code:bash (git fetch origin), code:bash (uv run pytest tests/ -q   # full suite; all pass), code:bash (git commit -m "feat(rabbit): BaseConsumer with ack/nack/dead), code:block12 (Co-Authored-By: DeepSeek <noreply@deepseek.com>) (+25 more)
...
Cohesion: 0.53
Nodes (4): Skeleton(), SkeletonProps, SkeletonRow(), SkeletonRowProps
...
Cohesion: 0.10
Nodes (21): AGENTS.md — Tomorrowland (compact), Architecture (what agents need to know), Branch flow, code:bash (uv run ruff check --fix src/ tests/ migrations/), code:bash (uv run pytest tests/unit/test_search_hybrid.py -q), code:bash (uv run pytest tests/unit/test_search_hybrid.py -q), code:text (main), code:bash (git diff --name-only <target-branch>...HEAD) (+13 more)

### Community 253 - "Module Group 253"
Cohesion: 0.60
Nodes (3): Badge(), BadgeProps, BadgeVariant
...
Cohesion: 0.60
Nodes (3): log_step(), require_command(), usage()
...
Cohesion: 0.70
Nodes (3): downgrade(), upgrade(), _uuid_pk()
...
Cohesion: 0.09
Nodes (11): IntelligenceConsumer, main(), ElasticsearchSearchClient, Index or update a document by *document_id*., Thin wrapper around the Elasticsearch client for full-text (BM25) search., Remove a document from the index., Update a single field of an existing document (partial update)., Update multiple fields of an existing document atomically. (+3 more)
...
Cohesion: 0.12
Nodes (14): C. File Support Matrix, Context Loaded, Context Skipped, GET /documents/{id}/converted-preview Endpoint, document_converted_previews Table, Document Viewer Design, GitHub Issues to Create, Later Enhancements (Post-MVP) (+6 more)
...
Cohesion: 0.22
Nodes (7): ACL Audit — Document-Derived API Surfaces (D1 #400 #142), D2 fix checklist, Handoff, HIGH, LOW, MEDIUM, Risk ranking
...
Cohesion: 0.06
Nodes (30): 4 passed, Acceptance Checklist, Allowed Changes, clean, code:bash (git fetch origin), code:block10 (feat(rabbit): Sub-A — RabbitMQ service, RabbitClient, topolo), code:markdown (## Summary), code:block12 (+22 more)
...
Cohesion: 0.36
Nodes (6): RequestTranslationDialog(), RequestTranslationDialogProps, invalidateSpy, onClose, qc, renderWithClient()
...
Cohesion: 0.06
Nodes (31): Acceptance Checklist, all passed, Allowed Changes, code:bash (git fetch origin), code:bash (uv run pytest tests/integration/test_admin_jobs_routes.py -q), code:bash (git commit -m "feat(rabbit): GET /admin/rabbit/queues route"), code:block12 (Co-Authored-By: DeepSeek <noreply@deepseek.com>), code:block13 (feat(rabbit): Sub-E — admin queue monitoring + job retry end) (+23 more)

### Community 494 - "Community 494"
Cohesion: 0.06
Nodes (31): Acceptance Checklist, all passed, Allowed Changes, clean, code:bash (git fetch origin), code:bash (git commit -m "feat(rabbit): retry tier exchange with 30s TT), code:block11 (Co-Authored-By: DeepSeek <noreply@deepseek.com>), code:block12 (feat(rabbit): Sub-F — retry tier exchange + Prometheus alert) (+23 more)
...
Cohesion: 0.29
Nodes (6): code:bash (uv run ruff check --fix src/ tests/ migrations/), code:bash (npm run lint), Constraints, Skill: Implement a Tomorrowland Issue, Steps, Stop conditions

### Community 504 - "Community 504"
Cohesion: 0.29
Nodes (6): code:bash (uv run ruff check src/ tests/ migrations/), code:block2 (@claude review this PR against AGENTS.md, the issue, archite), Review prompts, Skill: Review a Tomorrowland PR, Steps, Stop conditions
...
Cohesion: 0.44
Nodes (16): _create_annotation(), _create_doc(), _make_client(), Integration tests for annotation reply endpoints., _settings(), _setup_users(), test_annotations_list_includes_reply_count_field(), test_create_reply() (+8 more)

### Community 617 - "Community 617"
Cohesion: 0.21
Nodes (15): move_to_originals(), Original file storage — move connector-owned temp files into files_root/original, Return a durable storage path for *path* under ``files_root/originals/``.      R, _make_file(), Unit tests for services.pipeline.original_store.move_to_originals., test_already_inside_files_root_returns_as_is(), test_extension_preserved(), test_file_without_extension() (+7 more)
...
Cohesion: 0.12
Nodes (15): code:python ("""Intelligence stage consumer — wraps existing Intelligence), code:python ("""Alert stage consumer — runs AlertMatcher for a document."), code:bash (uv run mypy src/services/pipeline/intelligence_consumer.py \), code:bash (git add src/services/pipeline/intelligence_consumer.py \), code:bash (#!/usr/bin/env bash), code:bash (chmod +x scripts/validate-rabbitmq.sh), File Map, Final Validation Before Feature Branch → main (+7 more)
...
Cohesion: 0.15
Nodes (13): code:python ("""Add stage and rabbit_message_id to pipeline_jobs.), code:bash (uv run alembic heads), code:bash (uv run alembic upgrade head), code:bash (git add migrations/versions/), code:python (from unittest.mock import MagicMock, patch), code:bash (uv run pytest tests/unit/test_publisher.py -q), code:python (def set_rabbit_message_id(self, job_id: UUID, message_id: st), code:python ("""DocumentPublisher — writes DB state and publishes RabbitM) (+5 more)
...
Cohesion: 0.20
Nodes (10): code:python (from unittest.mock import MagicMock, patch), code:python ("""Admin routes for RabbitMQ queue observability."""), code:python (from services.api.routers.admin import rabbit as rabbit_rout), code:bash (git add src/services/api/routers/admin/rabbit.py \), code:python (@router.post("/admin/jobs/{job_id}/retry")), code:bash (uv run pytest tests/integration/test_admin_jobs_routes.py -q), code:bash (git add src/services/api/routers/admin/jobs.py), Sub-issue E — Admin Monitoring (#429) (+2 more)

### Community 637 - "Community 637"
Cohesion: 0.20
Nodes (9): Tests that page_number and section_heading flow through RAG citation assembly., Citation model fields must accept page_number and section_heading., When not provided, page_number and section_heading default to None., Verify Citation picks up page_number/section_heading from chunk metadata dict, Verify that _retrieve_chunks copies page_number and section_heading from     Sea, test_citation_model_accepts_page_number_and_section_heading(), test_citation_model_defaults_are_none(), test_citation_populated_from_metadata() (+1 more)

### Community 639 - "Community 639"
Cohesion: 0.22
Nodes (6): OcrExtractor, OCR extractor for raster image files using Tesseract.  Requires ``pytesseract``, Extract text from raster images via Tesseract OCR., Return OCR'd text from an image file.          Returns an empty ExtractionResult, Register the OCR extractor for raster image MIME types., test_ocr_extractor_calls_tesseract()
...
Cohesion: 0.48
Nodes (5): test_cors_origins_default_to_frontend_origin(), test_cors_origins_empty_values_are_ignored(), test_cors_origins_parse_comma_separated_values(), test_env_example_matches_settings_and_feature_defaults(), test_settings_load_from_environment()

### Community 650 - "Community 650"
Cohesion: 0.29
Nodes (7): code:python (from unittest.mock import MagicMock, patch), code:python (# Publish to RabbitMQ when enabled (no-op when disabled)), code:python (from shared.config import Settings), code:bash (uv run pytest tests/ -k "ingestion or sync_now" -q), code:bash (uv run pytest tests/ -q), code:bash (git add src/services/api/routers/admin/ingestion.py), Task 8: Wire RabbitMQ publish into sync-now

### Community 651 - "Community 651"
Cohesion: 0.29
Nodes (7): code:python ("""Integration tests for GET /admin/jobs and GET /admin/jobs), code:bash (uv run pytest tests/integration/test_admin_jobs_routes.py -q), code:python ("""Admin routes for pipeline job inspection and retry."""), code:python (from services.api.routers.admin import jobs as jobs_router), code:bash (uv run pytest tests/integration/test_admin_jobs_routes.py -q), code:bash (git add src/services/api/routers/admin/jobs.py \), Task 9: Admin jobs routes
...
Cohesion: 0.53
Nodes (4): admin_health(), admin_readiness(), app_health(), metrics()

### Community 656 - "Community 656"
Cohesion: 0.33
Nodes (6): code:python (from unittest.mock import MagicMock, call, patch), code:bash (uv run pytest tests/unit/test_rabbit_client.py -q), code:python ("""Thin RabbitMQ client wrapper for Tomorrowland document pi), code:bash (uv run pytest tests/unit/test_rabbit_client.py -q), code:bash (git add src/shared/rabbit.py tests/unit/test_rabbit_client.p), Task 5: Implement `shared/rabbit.py`
...
Cohesion: 0.40
Nodes (4): downgrade(), Add language_detected boolean column to documents., Remove language_detected column from documents., upgrade()

### Community 665 - "Community 665"
Cohesion: 0.40
Nodes (4): Vector indexing worker that processes ``vector_index_document`` jobs.  Claims du, Run ``run_vector_once`` in a loop until interrupted.      Emits a heartbeat gaug, Run ``run_vector_once`` in a loop until interrupted.      Emits a heartbeat gaug, run_vector_loop()
...
Cohesion: 0.50
Nodes (3): 3. Queue Architecture Recommendation, Job Table Schema (illustrative, not a migration), Rationale

### Community 671 - "Community 671"
Cohesion: 0.50
Nodes (3): Send a generate request to Ollama and return the response text.          Args:, Send a generate request to Ollama and return the response text.          Args:, Send a generate request to Ollama and return the response text.          Args:
...
Cohesion: 0.50
Nodes (4): code:bash (../tomorrowland-release-<version>/scripts/tomorrowland-airga), code:bash (../tomorrowland-release-<version>/scripts/upgrade-airgap.sh ), code:bash (../tomorrowland-release-<version>/scripts/tomorrowland-airga), Upgrade command
...
Cohesion: 0.67
Nodes (3): Backup command, code:bash (../tomorrowland-release-<version>/scripts/backup-airgap-data), code:bash (docker compose --env-file .env -f docker-compose.airgap.yml )

### Community 688 - "Community 688"
Cohesion: 0.67
Nodes (3): code:bash (sha256sum -c tomorrowland-release-<version>.tar.gz.sha256), code:bash (tar xzf tomorrowland-release-<version>.tar.gz), Required commands before upgrade
